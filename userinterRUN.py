...
def get_youtube_comments(api_key, video_id, max_count=1000):
    ...
    # Create a progress bar since fetching 1000 comments takes time
    ...
    while len(comments) < max_count:
        try:
            request = youtube.commentThreads().list(
                part="snippet",
                videoId=video_id,
                maxResults=100,  # Hard limit of API is 100 per page
            ...
            for item in response['items']:
                ...
                # Stop immediately if we reach the user's limit
                if len(comments) >= max_count:
                    break
            
            # Update progress bar
            progress = min(len(comments) / max_count, 1.0)
            ...
            st.error(f"Error fetching comment page: {e}")
            break
...
if analyze_btn:
    if not video_url:
        ...
        with st.spinner(f"Fetching comments for Video ID: {video_id}..."):
            try:
                # 1. Fetch Data
                video_title, raw_comments = get_youtube_comments(api_key, video_id, max_count=1000)
                ...
                if not raw_comments:
                    ...
                    # Display Metrics - FIXED: Show both positive and negative counts
                    m1, m2, m3, m4, m5 = st.columns(5)  # Added one more column
                    m1.metric("Total Comments", total)
                    m2.metric("Positive 🟢", pos_count)
                    m3.metric("Negative 🔴", neg_count)
                    m4.metric("Neutral 🔵", neu_count)  # Added neutral count
                    m5.metric("Positivity Index", f"{positivity_ratio:.1f}%")
                    ...
                    # 5. Data Table - FIXED: Configure column width for full comment display
                    st.subheader("📝 Comment Analysis")
                    
                    # Configure pandas display options to show full text
                    pd.set_option('display.max_colwidth', None)
                    pd.set_option('display.width', None)
                    
                    # Create a styled dataframe with better column configuration
                    styled_df = df[['Author', 'Sentiment', 'Confidence', 'Comment']].style.format({
                        'Confidence': '{:.2f}'
                    })
                    
                    # Display the dataframe with optimized column widths
                    st.dataframe(
                        styled_df,
                    ...
                    # Alternative approach: If the above doesn't work well, use expandable rows
                    st.markdown("---")
                    st.subheader("📋 Detailed Comments (Expandable)")
                    
                    for idx, row in df.iterrows():
                        with st.expander(f"Comment {idx+1}: {row['Author']} - {row['Sentiment']} ({row['Confidence']:.2f})"):
